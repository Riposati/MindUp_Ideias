	<?php
		...
		class AvaliacaoDAO{
			...
			public function getCamposIdeia($idIdeia){
				...
				$testePraSaberSeEdigito = is_numeric($idIdeia);

				if($testePraSaberSeEdigito){

					$resultado = $conexao->query("select * from ideiasusuarios where idideia = $idIdeia"); // pega todos os campos da ideia selecionada
					$count = $resultado->rowCount(); // serve pra contar quantas linhas vieram na consulta

					if(strcmp($idIdeia,"")!=0 && $count > 0){
						
						return $resultado;		
					}else{

						return NULL; /// essa ideia nao existe entra aqui
					}
				}
			}
